            ...
            <div class="col-md-12">
                <div class="card">
                    <div class="card-header">
                        <h3 class="card-title">Create New Night Premium</h3>
                    </div>
                    <!-- /.card-header -->
                    <div class="card-body">
                        @if (count($errors) > 0)
                        ...
                        <form action="{{ route('night-premium.store') }}" method="POST">
                            @csrf
                            <div class="row">
                                <div class="col-md-6">
                                ...
                                <div class="col-md-6">
                                    <div class="form-group">
                                        <label for="night_rate">Night Premium Rate</label>
                                        <input type="number" step="0.01" name="night_rate" id="night_rate" class="form-control" value="1.10" required>
                                        <small class="text-muted">1.10 for regular night premium</small>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="row">
                                <div class="col-md-6">
                                    <div class="btn-group" role="group" aria-label="Button group">
                                        ...
                                        <a href="{{ route('night-premium.index') }}" class="btn btn-info">Back</a>
                                    </div>
                                </div>
                            </div>
                        </form>
    ...
    <script>
        $(document).ready(function() {
            ...
            function updateDefaultTimes() {
                ...
                if (selectedDate) {
                    // Set default time_in to 10:00 PM on the selected date (typical night shift start)
                    const defaultTimeIn = selectedDate + 'T22:00';
                    $('#time_in').val(defaultTimeIn);
                    
                    // Set default time_out to 6:00 AM next day (typical night shift end)
                    // Calculate next day
                    const nextDay = new Date(selectedDate);
                    nextDay.setDate(nextDay.getDate() + 1);
                    const nextDayFormatted = nextDay.toISOString().split('T')[0];
                    const defaultTimeOut = nextDayFormatted + 'T06:00';
            ...
            $('form').on('submit', function(e) {
                ...
                if (timeIn) {
                    ...
                    if (timeInDate !== selectedDate) {
                        e.preventDefault();
                        alert('Time in date must match the selected night premium date.');
                    }
                }
            });
        });
