            ...
            <div class="card">
                <div class="card-header">
                    <div class="d-flex justify-content-between align-items-center flex-wrap">
                        ...
                        <div class="card-tools">
                            <a href="{{ route('night-premium.apply') }}" class="btn btn-primary btn-sm">
                            <i class="fas fa-plus-circle"></i> Apply New Night Premium
                            </a>
                        </div>
                    </div>
                </div>
                <div class="card-body">
                    ...
                    <div id="loading-spinner" class="text-center py-4">
                        ...
                        <p class="mt-2 text-muted">Loading night premium records...</p>
                    </div>

                    <div id="night-premium-data" style="display: none;">
                        @if($nightPremiumRecords->isEmpty())
                            <div class="alert alert-info">
                                You haven't submitted any night premium requests yet. Click the "Apply for New Night Premium" button to create your first request.
                            </div>
                        @else
                            <div class="table-responsive">
                                <table id="night-premium-history-table" class="table table-bordered table-hover">
                                    ...
                                    <tbody>
                                        @foreach($nightPremiumRecords as $record)
                                            <tr>
                                                <td>{{ date('M d, Y', strtotime($record->date)) }}</td>
                                                <td>{{ date('h:i A', strtotime($record->time_in)) }}</td>
                                                <td>{{ date('h:i A', strtotime($record->time_out)) }}</td>
                                                <td>{{ number_format($record->night_hours, 2) }}</td>
                                                <td>{{ number_format($record->night_rate, 2) }}x</td>
                                                <td class="text-success">₱{{ number_format($record->night_premium_pay, 2) }}</td>
                                                <td>
                                                ...
                                                <td class="text-center">
                                                    <button type="button" class="btn btn-sm btn-outline-primary view-details" 
                                                            ...
                                                            data-bs-target="#nightPremiumModal{{ $record->id }}"
                                                            data-id="{{ $record->id }}">
                                                        View
                                                    </button>
                                                </td>
                                            ...
                                            <!-- Modal for night premium details -->
                                            <div class="modal fade" id="nightPremiumModal{{ $record->id }}" tabindex="-1" 
                                                aria-labelledby="nightPremiumModalLabel{{ $record->id }}" aria-hidden="true">
                                                <div class="modal-dialog modal-lg">
                                                    <div class="modal-content">
                                                        <div class="modal-header">
                                                            <h5 class="modal-title" id="nightPremiumModalLabel{{ $record->id }}">
                                                                Night Premium Details
                                                            </h5>
                                                        ...
                                                        <div class="modal-body">
                                                            ...
                                                            <div class="row mb-3">
                                                                <div class="col-md-4">
                                                                    <strong>Hours:</strong> {{ number_format($record->night_hours, 2) }}
                                                                </div>
                                                                <div class="col-md-4">
                                                                    <strong>Rate:</strong> {{ number_format($record->night_rate, 2) }}x
                                                                </div>
                                                                <div class="col-md-4">
                                                                    <strong>Amount:</strong> ₱{{ number_format($record->night_premium_pay, 2) }}
                                                                </div>
                                                            </div>
                                                        ...
                                                        <div class="modal-footer">
                                                            ...
                                                            <a href="{{ route('night-premium.apply') }}" class="btn btn-primary">
                                                                Submit New Request
                                                            </a>
                                                            @endif
                                                        </div>
...
<script>
    $(document).ready(function() {
        ...
        const nightPremiumData = document.getElementById('night-premium-data');
        
        // Initialize DataTable
        let table = $('#night-premium-history-table').DataTable({
            responsive: true,
            language: {
                search: "_INPUT_",
        ...
        setTimeout(function() {
            ...
            nightPremiumData.style.display = 'block';
